...
class LogitNormLoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        return nn.CrossEntropyLoss()(normed_logits, targets)
...
def get_loss_function(loss1, loss2):
    ...
    return lambda outputs, targets: loss_dict[loss1](outputs, targets) + loss_dict[loss2](outputs, targets)
...
def main(args):
    ...
    model = train(args, model, loss_fn, loss_name, enc=True)

    # ✅ Train Decoder
    print("========== DECODER TRAINING ==========")
    model = train(args, model, loss_fn, loss_name, enc=False)
